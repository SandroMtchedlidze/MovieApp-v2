...
class MovieDetailsRepositoryImpl(
    ...
    override fun getMovieDetails(movieId: Int): Flow<ApiResult<MovieDetailsResponse>> {
        return responseHandler.apiCall { movieDetailsApi.getMovieDetails(movieId) }.map { result ->
            when (result) {
                is ApiResult.Loading -> result
                is ApiResult.Success -> ApiResult.Success(movieMapper.mapToDomain(result.data))
